...
- `galerij.html` + `src/rafts.ts` + `src/gallery.ts` + `src/gallery.css`: the gallery is the pond itself. Each photo (`.raft` button) floats on a log drawn on the canvas (`public/sprites/boomstam.png`); `rafts.ts` random-walks the rafts and makes them solid for birds and litter; press and hold a raft (log or photo) to drag it into another one and shove it aside (pointer events on `.raft`, suppressing the click if the drag moved more than a few pixels). `gallery.ts` handles the splash and lightbox. `src/pixel.ts` sets `--px` (one pond pixel in CSS px) so DOM sizes are whole pond pixels. Only as many rafts show as fit in 40% of the water; the lightbox still browses all photos. The gallery page is left out of the screenshot tests (everything on it moves randomly).
