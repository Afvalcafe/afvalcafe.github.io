...
- `galerij.html` + `src/rafts.ts` + `src/gallery.ts` + `src/gallery.css`: the gallery is the pond itself. Each photo (`.raft` button) floats on a log drawn on the canvas (`public/sprites/boomstam.png`); `rafts.ts` random-walks the rafts and makes them solid for birds, litter and lily pads (pushed out in `pond.ts`'s `separate()`); press and hold a raft (log or photo, past a small move threshold so a plain click still opens the lightbox) to drag it into another one — collisions push diagonally along the line between their centers, not just the smaller-overlap axis, and give both rafts a damped rocking tilt (`wobble`/`wobbleV`, rendered as `rotate()`), closer to logs nudging each other on water. `gallery.ts` handles the splash and lightbox. `src/pixel.ts` sets `--px` (one pond pixel in CSS px) so DOM sizes are whole pond pixels. Only as many rafts show as fit in 40% of the water; the lightbox still browses all photos. The gallery page is left out of the screenshot tests (everything on it moves randomly).
...
- `over-ons.html` + `src/about.ts` + `src/belt.ts` + `src/belt-sprites.ts` + `src/belt.css`: Over ons has a recycling game as its background: a top-view conveyor belt (absolute canvas `#belt` covering the whole document, so it scrolls with the page) snakes down beside and below the card into a side-view "Doom fire" at the bottom. Hover (mouse) or swipe (touch) over an item to knock it off in that direction; `knock()` aims it at the bin in that direction (paper, plastic, organic, glass, electronics). 3 correct items give sparkles, then a construction worker (random skin tone and gender) hammers the material into a product and cheers with it (books, rubber duck, plant, wine glasses, valuable earth metals). Items reaching the end burn with a red sad smiley; electronics (battery, phone) instead give a huge green explosion and turn the fire green for a while. Only recyclable items appear. Items without a PNG are pixel maps in `belt-sprites.ts`. If fewer than 5 bins fit (narrow screens), the canvas grows taller (more belt runs). A few litter pickers (like the people in the Doe mee game, not interactive) walk around on the grass; now and then small (half-size) litter appears on the grass, a different item each time, and they pick it up into their bag. They only walk where their legs are on free grass. After about half a minute each one walks off the side of the screen (hopping over any belt in the way) and a new one walks in to take over. An electronics explosion blows nearby pickers over; they get up again dizzy (circling stars). Flowers from the Doe mee game are scattered in the grass, and two half-size mallards (the pond sprite `eend.png` with drawn orange legs), kept apart, waddle about and peck. A touch that starts on an item cancels scrolling. Masked in the screenshot tests.
